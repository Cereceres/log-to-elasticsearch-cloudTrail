...
function transform(payload) {
    ...
    payload.logEvents.forEach((logEvent) => {
        ...
        const indexName = [
            `abg-cloudtrail-${ timestamp.getUTCFullYear()}`, // year
            `0${ timestamp.getUTCMonth() + 1}`.slice(-2), // month
            `0${ timestamp.getUTCDate()}`.slice(-2) // day
        ].join('.');

        const source = buildSource(logEvent.message, logEvent.extractedFields);
        if (source.userIdentity && source.userIdentity.principalId) {
            source.userIdentity.principalId = source
                .userIdentity.principalId.split(':').pop();
            if (source.userIdentity.arn) source.userIdentity.arn = source.userIdentity.arn.split('/').pop();
        }
